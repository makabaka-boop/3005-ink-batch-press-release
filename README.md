...
- 计划中的工单可换料：操作员在工单卡片选择另一活跃批次并确认，服务端在同一事务中校验工单仍为计划中且目标批次不同，按计划用量返还原批次并预占目标批次，同时更新工单关联；目标批次余额不足、已停用或工单已完成/取消时返回 409 具体冲突，工单归属与两边余额均保持原值，页面保留已选目标便于重新选择。换料成功后沿用创建工单的日期与质检规则为目标批次补充该批次尚不存在的对应问题，相同风险在不同批次各自生成独立检查记录，原问题保留不改动；工单卡片刷新后仍显示新批次、最近一次换料前批次与换料时间，列表响应追加最近换料信息，旧客户端无需提交新字段。
...
- 启动时自动为既有数据库补齐重量、工单状态与换料记录字段：历史批次按 `DEFAULT_RECEIVED_WEIGHT`（默认 100 kg）回填入库/可用重量，历史工单计划用量记 0、实际用量与完成时间保持为空并继续呈现计划中的未完成语义，不反向扣减；换料前批次与换料时间保持为空，视为从未换料；未携带新字段的旧请求按兼容默认值处理。
