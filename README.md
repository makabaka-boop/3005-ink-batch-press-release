...
- 计划中的工单可换料：操作员在工单卡片选择另一活跃批次并确认，服务端在同一事务中校验工单仍为计划中且目标批次不同，按计划用量返还原批次并预占目标批次，同时更新工单关联；目标批次余额不足、已停用或工单已完成/取消时返回 409 具体冲突，工单归属与两边余额均保持原值，页面保留已选目标便于重新选择。换料成功后沿用创建工单的日期与质检规则为目标批次补充尚不存在的对应问题，原问题作为当时检查记录保留；工单卡片刷新后仍显示新批次、最近一次换料前批次与换料时间，列表响应追加最近换料信息，旧客户端无需提交新字段。
