...
b.available_weight-=d.planned_usage;const j={id:state.jobs.length+1,...d,batch_code:b.code,batch_color:b.color,status:'planned',cancelled_at:null,created_at:'2026-09-10T08:00:00'};state.jobs.push(j);return Promise.resolve(ok({...j,issues_created:0}))}
...
j.status='cancelled';j.cancelled_at='2026-09-10T09:00:00';state.batches[0].available_weight+=j.planned_usage;return Promise.resolve(ok({id:j.id,status:'cancelled',returned_weight:j.planned_usage}))}
